...
<html <?php language_attributes(); ?>>
    <head>
        ...
        <link rel="stylesheet" type="text/css" href="<?php bloginfo('template_directory'); ?>/css/font-awesome-4.2.0/css/font-awesome.min.css">
...
<?php $logo_title = get_bloginfo('name') . ' | ' . ( is_home() || is_front_page() ? get_bloginfo('description') : wp_title('', false)); ?>
    ...
    <?php if (!empty($my_account_page)) { ?> <p><a href="<?php echo $my_account_page; ?>" title="<?php _e('My Account', 'bookyourtravel'); ?>"><?php _e('My Account', 'bookyourtravel'); ?></a>/<a class="fn" href="<?php echo wp_logout_url(home_url()); ?>"><?php _e('Sign out', 'bookyourtravel'); ?></a></p> <?php } ?><?php } ?>
                    </div>
